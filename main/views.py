...
class IndexView(CommonTemplateMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.headers.get('HX-Request'):
            return TemplateResponse(request, 'main/home_content.html', context=context)
        return TemplateResponse(request, self.template_name, context=context)


class CatalogView(CommonTemplateMixin, TemplateView):
    FILTER_MAPPING = {
    ...
    def get_context_data(self, **kwargs):
        ...
        if category_slug:
            ...
            products = products.filter(category=current_category)  # .order_by('-created_at')
...
class ProductDetailView(CommonTemplateMixin, DetailView):
    model = Product
    slug_field = 'slug'
    slug_url_kwarg = 'slug'
