...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            product_size = product.product_size.filter(stock__gt=0).first()
            if not product_size:
                return JsonResponse({
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'error': f'Cannot add {quantity} items. Only {product_size.stock - existing_item} more available.'
                }, status=400)

        cart_item = cart.add_product(product=product, product_size=product_size, quantity=quantity)
        request.session['cart_id'] = cart.id
        request.session.modified = True
        ...
        if request.headers.get('HX-Request'):
            ...
            return JsonResponse({
                ...
                'message': f'{product.name} add to cart',
...
class CartSummaryView(CartMixin, View):
    def get(self, request):
        ...
        context = {
            'cart': cart,
            'cart_items': cart.items.select_related('product', 'product_size__size').ored_by('-added_at')
        }
